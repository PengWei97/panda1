...
/**
 * Test4FiniteStrainPlasticMaterial implements rate-independent associative J2 plasticity
 ...
 */
class Test4FiniteStrainPlasticMaterial : public ComputeStressBase
{
public:
  static InputParameters validParams();

  Test4FiniteStrainPlasticMaterial(const InputParameters & parameters);
...
protected:
  ...
  MaterialProperty<Real> & _hard_factor;

  MaterialProperty<Real> & _eqv_plastic_function;
  ...
  std::string _hard_factor_name;

  /// vector of elasticity tensor material properties
  std::vector<MaterialProperty<Real> *> _D_hard_factor;  

  Real _length_scale;
  Real _pressure_scale;

  Real _yield_strength_init;
   ...
   */
  virtual Real dyieldFunction_dinternal(const Real equivalent_plastic_strain, const Real hard_factor);
   ...
   */
  Real getYieldStress(const Real equivalent_plastic_strain, const Real hard_factor);

  /**
   * d(yieldstress)/d(equivalent plastic strain)
   */
  Real getdYieldStressdPlasticStrain(const Real equivalent_plastic_strain, const Real hard_factor);
