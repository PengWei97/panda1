...
#include "TestFiniteStrainPlasticMaterial.h"
#include "libmesh/utility.h"

registerMooseObject("TensorMechanicsApp", TestFiniteStrainPlasticMaterial);

InputParameters
TestFiniteStrainPlasticMaterial::validParams()
{
...
TestFiniteStrainPlasticMaterial::TestFiniteStrainPlasticMaterial(const InputParameters & parameters)
...
void
TestFiniteStrainPlasticMaterial::initQpStatefulProperties()
{
...
void
TestFiniteStrainPlasticMaterial::computeQpStress()
{
 ...
 */
void
TestFiniteStrainPlasticMaterial::returnMap(const RankTwoTensor & sig_old,
...
Real
TestFiniteStrainPlasticMaterial::yieldFunction(const RankTwoTensor & stress, const Real yield_stress)
{
  return getSigEqv(stress) - yield_stress;
}

RankTwoTensor
TestFiniteStrainPlasticMaterial::dyieldFunction_dstress(const RankTwoTensor & sig)
{
...
Real
TestFiniteStrainPlasticMaterial::dyieldFunction_dinternal(const Real equivalent_plastic_strain)
{
...
RankTwoTensor
TestFiniteStrainPlasticMaterial::flowPotential(const RankTwoTensor & sig)
{
  return dyieldFunction_dstress(sig); // this plasticity model assumes associative flow
}

Real
TestFiniteStrainPlasticMaterial::internalPotential()
{
  return -1;
}

Real
TestFiniteStrainPlasticMaterial::getSigEqv(const RankTwoTensor & stress)
{
...
void
TestFiniteStrainPlasticMaterial::getJac(const RankTwoTensor & sig,
                                    const RankFourTensor & E_ijkl,
                                    Real flow_incr,
...
Real
TestFiniteStrainPlasticMaterial::getYieldStress(const Real eqpe)
{
  unsigned nsize;
  ...
  return value_0 + hard_factor*eqpe;
...
Real
TestFiniteStrainPlasticMaterial::getdYieldStressdPlasticStrain(const Real eqpe)
{
  unsigned nsize;
