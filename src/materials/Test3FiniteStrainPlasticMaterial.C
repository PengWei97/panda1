...
#include "Test3FiniteStrainPlasticMaterial.h"
#include "libmesh/utility.h"

registerMooseObject("TensorMechanicsApp", Test3FiniteStrainPlasticMaterial);

InputParameters
Test3FiniteStrainPlasticMaterial::validParams()
{
...
Test3FiniteStrainPlasticMaterial::Test3FiniteStrainPlasticMaterial(const InputParameters & parameters)
...
void
Test3FiniteStrainPlasticMaterial::initQpStatefulProperties()
{
  ComputeStressBase::initQpStatefulProperties();
  _plastic_strain[_qp].zero();
  _eqv_plastic_strain[_qp] = 0.0;
  _von_mises_stress[_qp] = 0;
  _hard_factor[_qp] = 2000;
}

void
Test3FiniteStrainPlasticMaterial::computeQpStress()
{

  _hard_factor[_qp] = computeHardFactor(); 
 ...
 */
void
Test3FiniteStrainPlasticMaterial::returnMap(const RankTwoTensor & sig_old,
...
Real
Test3FiniteStrainPlasticMaterial::yieldFunction(const RankTwoTensor & stress, const Real yield_stress)
{
  return getSigEqv(stress) - yield_stress;
}

RankTwoTensor
Test3FiniteStrainPlasticMaterial::dyieldFunction_dstress(const RankTwoTensor & sig)
{
...
Real
Test3FiniteStrainPlasticMaterial::dyieldFunction_dinternal(const Real equivalent_plastic_strain, const Real hard_factor)
{
...
RankTwoTensor
Test3FiniteStrainPlasticMaterial::flowPotential(const RankTwoTensor & sig)
{
  return dyieldFunction_dstress(sig); // this plasticity model assumes associative flow
}

Real
Test3FiniteStrainPlasticMaterial::internalPotential()
{
  return -1;
}

Real
Test3FiniteStrainPlasticMaterial::getSigEqv(const RankTwoTensor & stress)
{
...
void
Test3FiniteStrainPlasticMaterial::getJac(const RankTwoTensor & sig,
                                    const RankFourTensor & E_ijkl,
                                    Real flow_incr,
...
Real
Test3FiniteStrainPlasticMaterial::getYieldStress(const Real eqpe, const Real hf)
{
  unsigned nsize;
...
Real
Test3FiniteStrainPlasticMaterial::getdYieldStressdPlasticStrain(const Real eqpe, const Real hf)
{
  unsigned nsize;
...
Real
Test3FiniteStrainPlasticMaterial::computeHardFactor()
{
  // Get list of active order parameters from grain tracker
  const auto & op_to_grains = _grain_tracker.getVarToFeatureVector(_current_elem->id());

  Real sum_h = 0.0;
  std::vector<Real> hardFactor = {2500,3000};
  Real HardFactor = 0.0;
  ...
  for (MooseIndex(op_to_grains) op_index = 0; op_index < op_to_grains.size(); ++op_index)
  {
    ...
    HardFactor += hardFactor[op_index] * h;
    sum_h += h;
  }
