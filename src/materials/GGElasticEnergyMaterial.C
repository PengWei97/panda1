...
registerMooseObject("PhaseFieldApp", GGElasticEnergyMaterial);
...
Real
GGElasticEnergyMaterial::computeF()
{
  return 0.5 *(_elasticity_tensor[_qp] * _strain[_qp]).doubleContraction(_strain[_qp]);
}
...
Real
GGElasticEnergyMaterial::computeD2F(unsigned int i_var, unsigned int j_var)
{
  unsigned int i = argIndex(i_var);
