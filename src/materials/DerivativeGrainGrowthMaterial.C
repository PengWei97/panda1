...
#include "DerivativeGrainGrowthMaterial.h"

registerMooseObject("PhaseFieldApp", DerivativeGrainGrowthMaterial);

InputParameters
DerivativeGrainGrowthMaterial::validParams()
{
...
DerivativeGrainGrowthMaterial::DerivativeGrainGrowthMaterial(const InputParameters & parameters)
...
Real
DerivativeGrainGrowthMaterial::computeDF(unsigned int i_var)
{
...
Real
DerivativeGrainGrowthMaterial::computeD2F(unsigned int i_var, unsigned int j_var)
{
  const unsigned int i = argIndex(i_var);
  const int i_eta = _eta_index[i];
...
Real
DerivativeGrainGrowthMaterial::computeD3F(unsigned int i_var, unsigned int j_var, unsigned int k_var)
{
  const unsigned int i = argIndex(i_var);
  const int i_eta = _eta_index[i];
  const unsigned int j = argIndex(j_var);
