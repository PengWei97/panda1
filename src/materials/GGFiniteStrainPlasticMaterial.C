...
#include "GGFiniteStrainPlasticMaterial.h"
#include "libmesh/utility.h"

registerMooseObject("TensorMechanicsApp", GGFiniteStrainPlasticMaterial);

InputParameters
GGFiniteStrainPlasticMaterial::validParams()
{
...
GGFiniteStrainPlasticMaterial::GGFiniteStrainPlasticMaterial(const InputParameters & parameters)
  : ComputeStressBase(parameters),
    ...
    _hard_factor(declareProperty<Real>("hard_factor")),
...
void
GGFiniteStrainPlasticMaterial::initQpStatefulProperties()
{
...
void
GGFiniteStrainPlasticMaterial::computeQpStress()
{
 ...
 */
void
GGFiniteStrainPlasticMaterial::returnMap(const RankTwoTensor & sig_old,
...
Real
GGFiniteStrainPlasticMaterial::yieldFunction(const RankTwoTensor & stress, const Real yield_stress)
{
  return getSigEqv(stress) - yield_stress;
}

RankTwoTensor
GGFiniteStrainPlasticMaterial::dyieldFunction_dstress(const RankTwoTensor & sig)
{
...
Real
GGFiniteStrainPlasticMaterial::dyieldFunction_dinternal(const Real equivalent_plastic_strain)
{
  return -getdYieldStressdPlasticStrain(equivalent_plastic_strain);
}

RankTwoTensor
GGFiniteStrainPlasticMaterial::flowPotential(const RankTwoTensor & sig)
{
  return dyieldFunction_dstress(sig); // this plasticity model assumes associative flow
}

Real
GGFiniteStrainPlasticMaterial::internalPotential()
{
  return -1;
}

Real
GGFiniteStrainPlasticMaterial::getSigEqv(const RankTwoTensor & stress)
{
...
void
GGFiniteStrainPlasticMaterial::getJac(const RankTwoTensor & sig,
                                    const RankFourTensor & E_ijkl,
                                    Real flow_incr,
...
Real
GGFiniteStrainPlasticMaterial::getYieldStress(const Real eqpe)
{
  // unsigned nsize;
  Real value_0 = 700.0;
  // std::vector<Real> harden_factor = {1000.0, 2000.0};
  Real harden_factor = 1000.0;
  // Get list of active order parameters from grain tracker
  const auto & op_to_grains = _grain_tracker.getVarToFeatureVector(_current_elem->id());
  
  Real sum_h = 0.0;
  Real _hard_factor = 0.0;
  ...
  return value_0 + _hard_factor*eqpe;
}

Real
GGFiniteStrainPlasticMaterial::getdYieldStressdPlasticStrain(const Real eqpe)
{
  unsigned nsize;
