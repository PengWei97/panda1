...
#include "ComputePolycrystalElasticityTensorGG.h"
#include "RotationTensor.h"

registerMooseObject("PhaseFieldApp", ComputePolycrystalElasticityTensorGG);

InputParameters
ComputePolycrystalElasticityTensorGG::validParams()
{
...
ComputePolycrystalElasticityTensorGG::ComputePolycrystalElasticityTensorGG(
...
void
ComputePolycrystalElasticityTensorGG::computeQpElasticityTensor()
{
  // Get list of active order parameters from grain tracker
  const auto & op_to_grains = _grain_tracker.getVarToFeatureVector(_current_elem->id());
  // std::cout << "the type of op_to_grains is: " << typeid(op_to_grains).name() << std::endl;
  ...
  for (MooseIndex(op_to_grains) op_index = 0; op_index < op_to_grains.size(); ++op_index)
  {
    ...
    // std::cout << "op_index is: " << op_index << std::endl;

    // std::cout << "Grian_ID is: " << grain_id << std::endl;

    // std::cout << "invalid_id is: " << FeatureFloodCount::invalid_id << std::endl;
