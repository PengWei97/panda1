...
#include "GGGrainTrackerElasticity.h"
#include "GGEulerAngleProvider.h"
#include "RotationTensor.h"

registerMooseObject("PhaseFieldApp", GGGrainTrackerElasticity);

InputParameters
GGGrainTrackerElasticity::validParams()
{
...
GGGrainTrackerElasticity::GGGrainTrackerElasticity(const InputParameters & parameters)
  : GrainDataTracker<RankFourTensor>(parameters),
    ...
    _euler(getUserObject<GGEulerAngleProvider>("euler_angle_provider"))
{
}

RankFourTensor
GGGrainTrackerElasticity::newGrain(unsigned int new_grain_id)
{
  EulerAngles angles;
  Real hardfactor;
  ...
  if (new_grain_id < _euler.getGrainNum())
  {
    ...
    hardfactor = _euler.getHardFactor(new_grain_id);
  }
  else
  {
    if (_random_rotations)
    {
      ...
      mooseError("GGGrainTrackerElasticity has run out of grain rotation data.");
  }
